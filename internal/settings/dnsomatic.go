...
func NewDNSOMatic(data json.RawMessage, domain, host string, ipVersion models.IPVersion, noDNSLookup bool) (s Settings, err error) {
...
func (d *dnsomatic) isValid() error {
	switch {
	case !constants.MatchDNSOMaticUsername(d.username):
		return fmt.Errorf("username %q does not match DNS-O-Matic username format", d.username)
	case !constants.MatchDNSOMaticPassword(d.password):
